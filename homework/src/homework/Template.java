...
//		add image 
		...
		ImageIcon idIcon = new ImageIcon("src/homework/icons/id.png");
		Image idImage = idIcon.getImage(); // transform it 
		Image newIdImg = idImage.getScaledInstance(40, 40,  java.awt.Image.SCALE_DEFAULT); // scale it the smooth way  
		idIcon = new ImageIcon(newIdImg);  // transform it back
		
		
//		ADD 
		{
		JTextField jtf = new JTextField(20);
		TextPrompt tp = new TextPrompt("아이디 ", jtf);
		Border newBorder = BorderFactory.createMatteBorder(0, 0, 1, 0, Color.gray);
		jtf.setBorder(newBorder);
		jtf.setOpaque(false);
		tp.setForeground(Color.gray);
		tp.changeAlpha(0.5f);
		tp.changeStyle(Font.BOLD + Font.ITALIC);
		
		jp.add(new JLabel(idIcon));
		jp.add(jtf);
