 ...
 * 	이벤트 소스		: 이벤트가 발생되는 컴포넌트 (버튼, 체크박스, 서브밋, ...)
 * 	이벤트 리스너	: 이벤트를 감지, 이벤트가 발생되면 실제적으로 이벤트를 처리할 수 있도록 만든 인터페이스
 *	이벤트 핸들러	: 리스너에 전달된 이벤트를 실제로 처리할 수 있도록 이벤트 리스너에 
 *				  포함되어 있는 메소드 (추상메소드)
 *				  즉, 발생된 이벤트 객체를 받아서 실제적으로 처리해주는 기능을 하는 메소드 */

/*	ActionEvent	: 컴포넌트를 눌렀거나, 선택되었을때, ENTER키를 눌렀을 때 발생하는 이벤트
* 	ActionListener	: actionPerformed() 라는 추상 메소드를 가지고 있다
