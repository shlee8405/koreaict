package day16.com.ict.edu;

public class Ex01_Enum {
/*	열거형 (enum, Enumeration Type) : 상수를 하나의 객체로 인식하고, 여려 개의 상수 
 * 					객체들을 한곳에 모아둔 묶음
 * 
 */
	int su = 12;
	static final int JAVA = 200;
	static final int HTML = 300;
	
	public void play() {
//		지역 변수로는 static 변수 선언 불가. instance 메소드 안에 static 변수 선언 불가
//		static final int CSS = 500;
	}
	
//	영거형
	public enum Lesson {
		JAVA, JSP, SPRING, ANDROID, HTML
	}
	
	
	public static void main(String[] args) {
//		static 메소드에서는 전역변수(Instance)를 못쓴다
//		지역 변수는 사용 가능
//		하지만 static 메소드에서는 static 변수 사용 가능
		Ex01_Enum ex01 = new Ex01_Enum();

		int num = 100;
		
//		지역 변수로는 static 변수 선언 . instance 메소드 안에 static 변수 선언 불가
//		static final int JSP = 400;
		
		
		System.out.println(ex01.su);
		System.out.println(num);
	
		System.out.println(JAVA);
		System.out.println(HTML);
		
//		열거형 안에 있는 JAVA 추출
		Lesson s1 = Lesson.JAVA;
		System.out.println(s1);

//		한번에 다 추출
		Lesson[] items = Lesson.values();
//		ordinal() => index()
		for (int i = 0; i < items.length; i++) {
			System.out.println(items[i]+ " : " + items[i].ordinal());
		}
	
	}

}



























































